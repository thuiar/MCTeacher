...
class Meta_Correction(nn.Module):
    def __init__(self,input_dim,output_dim=1):
        super(Meta_Correction,self).__init__()
        self.meta_net = nn.Sequential(
            nn.Linear(input_dim,input_dim),
            nn.ReLU(),
            nn.Linear(input_dim,output_dim),
        )
    def forward(self,feature_unimodal,feature_multimodal):
        feature_ = torch.cat([feature_unimodal+feature_multimodal],dim=1)
        correlation_result = self.meta_net(feature_)
...
class V1_Semi(nn.Module):
    def __init__(self, args):
        ...
        self.meta_t = Meta_Correction(2*self.post_fusion_dim,output_dim=1)
        self.meta_a = Meta_Correction(2*self.post_fusion_dim,output_dim=1)
        self.meta_v = Meta_Correction(2*self.post_fusion_dim,output_dim=1)
        ...
        self.Specific_t = nn.Linear(self.post_text_dim, self.post_fusion_dim)
        ...
        self.Specific_a = nn.Linear(self.post_audio_dim, self.post_fusion_dim)
        ...
        self.Specific_v = nn.Linear(self.post_video_dim, self.post_fusion_dim)
    ...
    def forward(self, text_x, audio_x, video_x):
        ...
        x_t3 = F.relu(self.Specific_t(x_t2), inplace=True)
        # audio
        x_a1 = self.post_audio_dropout(audio_h)
        x_a2 = F.relu(self.post_audio_layer_1(x_a1), inplace=True)
        x_a3 = F.relu(self.Specific_a(x_a2), inplace=True)
        ...
        x_v3 = F.relu(self.Specific_v(x_v2), inplace=True)
        ...
        related_tv = fusion_t_with_v - x_t3 - x_v3
        ...
        related_ta = fusion_t_with_a - x_t3 - x_a3
        ...
        related_va = fusion_v_with_a - x_v3 - x_a3
        ...
        fusion_data = fusion_data + related_tv +related_ta+related_va
        fusion_data = self.post_fusion_layer_2(fusion_data)
        feature_m =fusion_data
        fusion_data = self.post_fusion_layer_3(fusion_data)
        output_fusion=fusion_data
        # output_fusion = torch.sigmoid(fusion_data)
#   * self.output_range+ self.output_shift
        
        final_M = output_fusion
        res = {
            'Feature_t': text_h,
            'Feature_a': audio_h,
            'Feature_v': video_h,
